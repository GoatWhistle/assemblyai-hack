...
### 2а. Эхо-петля: агент слышит себя и перебивает сам себя

Это не теория. На этом уже провалился участник хакатона: терминальный клиент claim-intake-agent предупреждает, что **без наушников агент слышит себя и перебивает каждую реплику** — AEC там нет вовсе, и судья на macOS этот путь просто не пройдёт ([разбор](../competitors/claim-intake.md)).

Механика отказа: агент говорит через колонки → микрофон захватывает его собственный голос → `input.speech.started` срабатывает от голоса агента → плейбек флешится → агент замолкает сам от себя. В нашем случае хуже: STT-сокет отдаст фантомный `Turn`, и в провенанс попадут слова, которые пользователь не произносил.

Три слоя защиты, и нужны все три:
...
Первый слой — браузерный AEC через `getUserMedia`. Обязателен, но недостаточен: он подавляет, а не устраняет, и на громкой колонке остаток проходит.

Второй слой — **узел захвата не роутится на выход**. `source.connect(worklet)` и всё; никакого `worklet.connect(audioCtx.destination)`. Ошибка выглядит безобидно (люди дописывают её, чтобы «услышать себя»), а даёт прямую петлю. Ровно это отдельно отмечено как сделанное правильно у VoiceMed: ворклет захвата никогда не роутится на колонки.

Третий слой — **гейт на время речи агента**. Пока агент говорит, аудио в STT-сокет всё равно идёт (иначе потеряем реальное перебивание), но ходы, пришедшие в окне воспроизведения, помечаются как подозрительные и не участвуют в провенансе, если совпадают с текстом собственной реплики агента. Дешёвая проверка: держать последнюю строку `transcript.agent` и сверять с пришедшим `Turn`.

Для чистоты замеров латентности всё это критично отдельно: фантомный ход сдвигает turn-to-turn на величину, которая не имеет отношения к API.
