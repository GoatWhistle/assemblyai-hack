...
sealed_files() {
  find "$HELDOUT" -type f ! -name '.gitkeep' ! -path "$HELDOUT/audio/*" \
    ! -name 'manifest.json' ! -name 'result-plain.json' ! -name 'result.json' "$@"
}

count=$(sealed_files | wc -l | tr -d ' ')
...
if [ ! -f "$SEAL" ]; then
  ...
  echo "seal it once with: bash scripts/checks/heldout-seal.sh --seal" >&2
  exit 1
fi
